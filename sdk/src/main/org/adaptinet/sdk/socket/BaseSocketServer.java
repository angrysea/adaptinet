...
abstract public class BaseSocketServer {
	...
	public void initialize(IServer theServer, int port, int sockets)
			throws Exception {
		...
		threadcache = new ThreadCache("sockets", sockets, server
				...
				while (running == true) {
					...
					if (free >= idealFree) {
						...
						if ((loadavg = AVG_NORMAL) < oldavg) {
							server.getThread().setPriority(
									Thread.MAX_PRIORITY);
						}
					} else if (free > 0) {
						if ((loadavg = AVG_HIGH) > oldavg) {
							server.getThread().setPriority(
									server.getClientThreadPriority() - 2);
						}
					} else {
						loadavg = AVG_DEAD;
					}
	...
	static final public BaseSocketServer createInstance(String type,
		...
		try {
			if (type.startsWith("PROXY")) {
				c = (Class<BaseSocketServer>)Class.forName("org.adaptinet.sdk.socket.ProxySocketServer");
				baseType = type;
			} else if (type.startsWith("CALLBACK")) {
				c = (Class<BaseSocketServer>)Class.forName("org.adaptinet.sdk.socket.CallbackSocketServer");
				baseType = type;
				...
				c = (Class<BaseSocketServer>)Class
						.forName("org.adaptinet.sdk.socket.SecureHttpSocketServer");
				baseType = type.substring(3);
			} else if (type.startsWith("CUSTOM")) {
				c = (Class<BaseSocketServer>)Class.forName(serverclass);
				baseType = type;
			} else {
				c = (Class<BaseSocketServer>)Class.forName("org.adaptinet.sdk.socket.HttpSocketServer");
				baseType = type;
			}
