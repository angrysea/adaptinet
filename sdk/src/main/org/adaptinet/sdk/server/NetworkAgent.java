...
import org.adaptinet.sdk.node.NodeRoot;
...
public final class NetworkAgent implements IResetEvent {
	...
	public NetworkAgent(final IServer server, final String name,
	...
	/**
	 * This is called by the server so that the network agent can inform
	 * all nodes it is directly connected too that it is dropping out of the
	 * network.
	 */
	...
	public void broadcastMessage(final Envelope env) throws AdaptinetException {
		...
		if (bConnected == false) {
			AdaptinetException adaptinetex = new AdaptinetException(
