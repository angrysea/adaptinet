...
public class ServerConfiguration {
	static public void setConfiguration(IServer serve,
			String strRequest) {
		...
		serve.saveConfig(properties);
	}

	static public String getConfiguration(IServer serve) {
		return getConfiguration(serve, true);
	}
	...
	static public String getConfiguration(IServer serve,
		...
		try {
			...
			buffer.append("<INPUT type=\"hidden\" id=\"command\" name=\"command\" value=\"serveconfig\"/>");
			...
			buffer.append("<tr><font class=\"header\">Server Settings</font></td></tr>");

			Properties properties;
			if (bFromFile == true) {
				properties = serve.getConfigFromFile();
			} else {
				properties = serve.getConfig();
			}
