package org.adaptinet.sdk.samples.chat;
...
public class Chat extends Processor {
	...
	public void init() {
		 ...
		 * connection takes longer than 3 seconds we are probably the only one
		 * in the network.
		 */
	...
	/**
	 * This method is called when nodes are added or removed from the network
	 */
	public void nodeUpdate() {

		if (!bConnected) {
			/**
			 * Tell the world that I have joined the Chat network. Obviously you
			 * would want to change this if you are expecting a large network of
			 * users.
			 */
			if (myUsername != null && !myUsername.isEmpty()) {
				frame.resetUserList(usersMap);
				try {
					broadcastMessage("/Chat/joinRequest", myUsername,
							Address.getMyURL());
	...
	/**
	 * This method is called by a remote transceiver to notify you that they
	 * would like to join the Chat network
	 * 
	 * @param name
	 * @param address
	 */
	public void joinRequest(String name, String address) {
		try {
			/**
			 * Do we already have a member with the same name but a different
			 * address?
			 */
			...
			if (mapAddress != null) {
				if (mapAddress.equals(address)) {
					/**
					 * If the name is the same, then we already have this member
					 * in the list.
					 */
					return;
				} else {
			 ...
			 */
			try {
				postMessage(address + "/Chat/currentMember", myUsername,
						Address.getMyURL());
			} catch (AdaptinetException e) {
				e.printStackTrace();
			}
	...
	/**
	 * This method is called by a remote transceiver to notify you that they are
	 * logging off.
	 * 
	 * @param name
	 * @param address
	...
	/**
	 * This method is called by a remote transceiver notifying you that they are
	 * already a member of the network. This gives you the ability to update
	 * your member list.
	 * 
	 * @param name
	 * @param address
