...
import org.adaptinet.sdk.adaptinetex.*;
...
public class Filer extends org.adaptinet.sdk.processoragent.Processor {
	...
	public void init() {
		...
		// Create a synchronized list of nodes that acknowledged that they
		// have the file
	...
	public void getFileButton(String fileName) {
		try {
			...
			Message message = new Message(
					"http://localhost:8083/Filer/queryFile", this.server);
			// Message message = new
			// Message("http://localhost:8083/Filer/queryFile",
			// this.server);

			// We're going to broadcast our message to our child nodes and
			// their child nodes. That should be enough.
			message.setHops("3");
	...
	private void requestFilePackets() {
		try {
			frame.setStatus("Requesting the file from the node list...");
			...
			while (it.hasNext()) {
				...
				Message message = new Message("http://"
						...
						+ "/Filer/getFilePacket", this.server);
	...
	public void queryFile(String fileName) {
		try {
			...
			Message message = new Message("http://" + sender.getHost() + ":"
					+ sender.getPort() + "/Filer/fileFound", this.server);
	...
	public void getFilePacket(String fileName, Long startPos, Long size) {
		try {
			...
			Address sender = msg.getReplyTo();
			Message message = new Message("http://" + sender.getHost() + ":"
					+ sender.getPort() + "/Filer/packetSent", this.server);
